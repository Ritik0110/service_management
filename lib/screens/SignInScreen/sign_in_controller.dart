...
class SignInController extends GetxController {
  ...
  Future<void> loadStoredData() async {
    ...
    errorMessage.value = "";
    isRememberMe.value = preferences.getBool(AppPreferences.isRememberMe) ?? false;
    if(isRememberMe.value){
     emailController.text = preferences.getString(AppPreferences.storedEmpId) ?? '';
     passwordController.text = preferences.getString(AppPreferences.storedEmpPass) ?? '';
    }else{
  ...
  Future<void> signIn() async {
    ...
    if(loginResult == "1"){
      ...
      prefs.setString(AppPreferences.storedEmpData, jsonEncode(AppVariables.employeeModel.toJson()));
